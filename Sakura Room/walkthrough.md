```markdown
# 🌸 Sakura Room — OSINT Investigation Write-up

This is a documentation of my OSINT investigation in the **Sakura Room** on TryHackMe, created by OSINT Dojo.  
...
### 🔍 Background
A static image was left by the attacker. Inspecting the page may reveal metadata containing the attacker’s identity.
...
**Notes:**  
- The binary in `aria-label` is decorative and not relevant.  
- Metadata leaks like file paths can reveal usernames.
...
1. Start with the known username: `SakuraSnowAngelAiko`.  
...
- **Full email address:** `SakuraSnowAngel83@protonmail.com`

### 💡 Notes
- Username correlation is a core OSINT technique.  
- Emails can appear in public keys, Git commits, or profile metadata.  
- Always verify accounts to avoid false positives.
